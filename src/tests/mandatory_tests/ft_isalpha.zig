...
fn ft_isalpha(ch: c_int) c_int {
    if (comptime !is_function_defined) {
        return 0;
    } else {
        return c.ft_isalpha(ch);
    }
}
...
fn test_alpha_chars_fn(_: std.mem.Allocator) TestCaseError!void {
    // Test lowercase letters
    try assert.expect(ft_isalpha('a') != 0, "Expected 'a' to be alphabetic");
    try assert.expect(ft_isalpha('z') != 0, "Expected 'z' to be alphabetic");
    try assert.expect(ft_isalpha('m') != 0, "Expected 'm' to be alphabetic");
...
fn test_non_alpha_chars_fn(_: std.mem.Allocator) TestCaseError!void {
    // Test digits
    try assert.expect(ft_isalpha('0') == 0, "Expected '0' to be non-alphabetic");
    try assert.expect(ft_isalpha('9') == 0, "Expected '9' to be non-alphabetic");
    try assert.expect(ft_isalpha('5') == 0, "Expected '5' to be non-alphabetic");

    // Test special characters
    try assert.expect(ft_isalpha('!') == 0, "Expected '!' to be non-alphabetic");
    try assert.expect(ft_isalpha('@') == 0, "Expected '@' to be non-alphabetic");
    try assert.expect(ft_isalpha('#') == 0, "Expected '#' to be non-alphabetic");
    try assert.expect(ft_isalpha(' ') == 0, "Expected ' ' to be non-alphabetic");
    try assert.expect(ft_isalpha('\t') == 0, "Expected '\\t' to be non-alphabetic");
    try assert.expect(ft_isalpha('\n') == 0, "Expected '\\n' to be non-alphabetic");
}
...
fn test_edge_cases_fn(_: std.mem.Allocator) TestCaseError!void {
    ...
    try assert.expect(ft_isalpha('@') == 0, "Expected '@' (ASCII 64, before 'A') to be non-alphabetic");
    try assert.expect(ft_isalpha('?') == 0, "Expected '?' (ASCII 63, before 'A') to be non-alphabetic");

    // Characters just after 'Z' (ASCII 90)
    try assert.expect(ft_isalpha('[') == 0, "Expected '[' (ASCII 91, after 'Z') to be non-alphabetic");
    try assert.expect(ft_isalpha('\\') == 0, "Expected '\\' (ASCII 92, after 'Z') to be non-alphabetic");

    // Characters just before 'a' (ASCII 97)
    try assert.expect(ft_isalpha('`') == 0, "Expected '`' (ASCII 96, before 'a') to be non-alphabetic");
    try assert.expect(ft_isalpha('_') == 0, "Expected '_' (ASCII 95, before 'a') to be non-alphabetic");

    // Characters just after 'z' (ASCII 122)
    try assert.expect(ft_isalpha('{') == 0, "Expected '{' (ASCII 123, after 'z') to be non-alphabetic");
    try assert.expect(ft_isalpha('|') == 0, "Expected '|' (ASCII 124, after 'z') to be non-alphabetic");
}
...
fn test_comparison_with_standard_fn(_: std.mem.Allocator) TestCaseError!void {
    for (0..127) |ch| {
        const custom_result = ft_isalpha(@intCast(ch)) != 0;
