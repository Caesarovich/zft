...
fn ft_strrchr(str: [*c]const u8, ch: c_int) [*c]u8 {
    if (comptime !is_function_defined) {
        return null;
    } else {
        return c.ft_strrchr(str, ch);
    }
}
...
fn test_char_present_fn(_: std.mem.Allocator) TestCaseError!void {
    ...
    const ch: u8 = 'W';
    const result = ft_strrchr(str, ch);
...
var test_strrchr_positive_wrapped = TestCase{
    .name = "strrchr with positive values (wrapped)",
    .fn_ptr = &test_strrchr_positive_wrapped_fn,
};

fn test_strrchr_positive_wrapped_fn(_: std.mem.Allocator) TestCaseError!void {
    const str1 = "Hello";
    const result1 = ft_strrchr(str1, 'e' + 256); // 'e' is 101, 101 + 256 = 357
    try assert.expect(result1 != null, "ft_strrchr should find character 'e' when searching with 'e' + 256");
    try assert.expect(@intFromPtr(result1) == @intFromPtr(&str1[1]), "Expected pointer to 'e' in the string");

    const str2 = "World";
    const result2 = ft_strrchr(str2, 'o' + 256); // 'o' is 111, 111 + 256 = 367
    try assert.expect(result2 != null, "ft_strrchr should find character 'o' when searching with 'o' + 256");
    try assert.expect(@intFromPtr(result2) == @intFromPtr(&str2[1]), "Expected pointer to 'o' in the string");

    const str3 = "ZigLang";
    const result3 = ft_strrchr(str3, 'L' + 256); // 'L' is 76, 76 + 256 = 332
    try assert.expect(result3 != null, "ft_strrchr should find character 'L' when searching with 'L' + 256");
...
var test_strrchr_negative_wrapped = TestCase{
    .name = "strrchr with negative values (wrapped)",
    .fn_ptr = &test_strrchr_negative_wrapped_fn,
};

fn test_strrchr_negative_wrapped_fn(_: std.mem.Allocator) TestCaseError!void {
    const buffer1 = "Hello";
    const result1 = ft_strrchr(buffer1, 'e' - 256); // 'e' is 101, 101 - 256 = -155
    try assert.expect(result1 != null, "ft_strrchr should find character 'e' when searching with 'e' - 256");
    try assert.expect(@intFromPtr(result1) == @intFromPtr(&buffer1[1]), "Expected pointer to 'e' in the string");

    const buffer2 = "World";
    const result2 = ft_strrchr(buffer2, 'o' - 256); // 'o' is 111, 111 - 256 = -145
    try assert.expect(result2 != null, "ft_strrchr should find character 'o' when searching with 'o' - 256");
    try assert.expect(@intFromPtr(result2) == @intFromPtr(&buffer2[1]), "Expected pointer to 'o' in the string");

    const buffer3 = "ZigLang";
    const result3 = ft_strrchr(buffer3, 'L' - 256); // 'L' is 76, 76 - 256 = -180
    try assert.expect(result3 != null, "ft_strrchr should find character 'L' when searching with 'L' - 256");
...
var test_cases = [_]*TestCase{
    ...
    &test_strrchr_positive_wrapped,
    &test_strrchr_negative_wrapped,
};
