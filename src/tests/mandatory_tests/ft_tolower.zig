...
fn ft_tolower(ch: c_int) c_int {
    if (comptime !is_function_defined) {
        return 0;
    } else {
        return c.ft_tolower(ch);
    }
}
...
fn test_uppercase_chars_fn(_: std.mem.Allocator) TestCaseError!void {
    try assert.expect(ft_tolower('A') == 'a', "Expected 'A' to convert to 'a'");
    try assert.expect(ft_tolower('M') == 'm', "Expected 'M' to convert to 'm'");
    try assert.expect(ft_tolower('Z') == 'z', "Expected 'Z' to convert to 'z'");
}
...
fn test_lowercase_chars_fn(_: std.mem.Allocator) TestCaseError!void {
    try assert.expect(ft_tolower('a') == 'a', "Expected 'a' to remain 'a'");
    try assert.expect(ft_tolower('m') == 'm', "Expected 'm' to remain 'm'");
    try assert.expect(ft_tolower('z') == 'z', "Expected 'z' to remain 'z'");
}
...
fn test_non_alpha_chars_fn(_: std.mem.Allocator) TestCaseError!void {
    try assert.expect(ft_tolower('0') == '0', "Expected '0' to remain '0'");
    try assert.expect(ft_tolower('9') == '9', "Expected '9' to remain '9'");
    try assert.expect(ft_tolower('!') == '!', "Expected '!' to remain '!'");
    try assert.expect(ft_tolower('@') == '@', "Expected '@' to remain '@'");
    try assert.expect(ft_tolower(' ') == ' ', "Expected ' ' to remain ' '");
}
...
fn test_edge_cases_fn(_: std.mem.Allocator) TestCaseError!void {
    ...
    try assert.expect(ft_tolower('@') == '@', "Expected '@' (char before 'A') to remain unchanged");
    try assert.expect(ft_tolower('`') == '`', "Expected '`' (char before 'a') to remain unchanged");

    // Characters just after 'Z'
    try assert.expect(ft_tolower('[') == '[', "Expected '[' (char after 'Z') to remain unchanged");
    try assert.expect(ft_tolower('{') == '{', "Expected '{' (char after 'z') to remain unchanged");
}
...
fn test_standard_comparison_fn(_: std.mem.Allocator) TestCaseError!void {
    for (0..255) |i| {
        const custom_result = ft_tolower(@intCast(i));
