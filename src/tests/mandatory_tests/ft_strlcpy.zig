...
fn test_normal_copy_fn(_: std.mem.Allocator) TestCaseError!void {
...
fn test_truncated_copy_fn(_: std.mem.Allocator) TestCaseError!void {
...
fn test_empty_string_copy_fn(_: std.mem.Allocator) TestCaseError!void {
    var dest: [10]u8 = undefined;
    const src = "";
    const result = ft_strlcpy(&dest[0], src, dest.len);
...
fn test_copy_size_zero_fn(_: std.mem.Allocator) TestCaseError!void {
    var dest = "TEST";
    const src = "Hello";
...
fn test_copy_with_dest_size_smaller_fn(_: std.mem.Allocator) TestCaseError!void {
    var dest: [10]u8 = undefined;
...
fn test_copy_with_size_larger_fn(_: std.mem.Allocator) TestCaseError!void {
    var dest: [20]u8 = undefined;
    for (&dest) |*b| {
        b.* = 'A';
    }
