...
fn test_alnum_chars_fn() AssertError!void {
    ...
    try assert.expect(c.ft_isalnum('m') != 0, "Expected 'm' to be alphanumeric");

    // Test numeric characters
    try assert.expect(c.ft_isalnum('0') != 0, "Expected '0' to be alphanumeric");
    try assert.expect(c.ft_isalnum('5') != 0, "Expected '5' to be alphanumeric");
    try assert.expect(c.ft_isalnum('9') != 0, "Expected '9' to be alphanumeric");
}
...
fn test_non_alnum_chars_fn() AssertError!void {
    ...
    try assert.expect(c.ft_isalnum(' ') == 0, "Expected ' ' to be non-alphanumeric");
    try assert.expect(c.ft_isalnum('\t') == 0, "Expected '\\t' to be non-alphanumeric");
    try assert.expect(c.ft_isalnum('\n') == 0, "Expected '\\n' to be non-alphanumeric");
}
...
fn test_edge_cases_fn() AssertError!void {
    ...
    try assert.expect(c.ft_isalnum(':') == 0, "Expected ':' (char after '9') to be non-alphanumeric");

    // Characters just before 'A'
    try assert.expect(c.ft_isalnum('@') == 0, "Expected '@' (char before 'A') to be non-alphanumeric");

    // Characters just after 'Z'
    try assert.expect(c.ft_isalnum('[') == 0, "Expected '[' (char after 'Z') to be non-alphanumeric");
